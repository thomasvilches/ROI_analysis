...
#### nonvac doesn't change the behaviour, vaccinated individuals go to a higher level, but oscilates with nonvac, waning immunity both vaccine and rec
dc = [1;map(y-> 79+y,0:5);map(y->125+y,0:10);map(y->171+y,0:13);map(y->208+y,0:40)]
rc = [1.0;map(y-> 1.0-(0.07/6)*y,1:6);map(y-> 0.93-(0.077/10)*y,1:11);map(y-> 0.853+(0.04/14)*y,1:14);map(y-> 0.893-(0.29/41)*y,1:41)]
run_param_scen_cal(true,0.1088,"newyorkcity",30,1,1,1,1,1,1,rc,dc,300,true)
